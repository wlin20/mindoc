...
$(function () {
    ...
    let changeTheme = {
        ...
        click () {
            let themes = ["classic","dark"]
            let theme = this.theme
            let next = 0
            themes.forEach(function (th,index){
                if(theme.indexOf(th) != -1){
                    next = (index + 1) % themes.length
                    return
                }
            })
            this.theme = themes[next]
            window.editor.setTheme(this.theme,this.theme)
        }
    };
    // editor 的初始化参数
    let initConfig  ={
        ...
        toolbar:[
            ...
            "upload", "record", "table", "|",
            "undo","redo","|",
            "content-theme","code-theme",changeTheme,"|",
            "export", "fullscreen", "both", "outline","br",
            {
                name: "more",
                toolbar: [
                    "br",
        ...
        hint: {
            delay: 200,
            headings:"标题",
            emojiPath: "https://cdn.jsdelivr.net/npm/vditor@3.8.0/dist/images/emoji",
            extend: [],
        },
    ...
    window.editorTool.wrapEditor(editor,initConfig,"markdown.vditor",editor.setValue,editor.insertValue,editor.getValue,editor.getHTML)
